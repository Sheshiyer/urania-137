...
- 2026-09-21: **v0.7.1 deployed to production — instrument-shell redesign + TypeSafe Jev integration live.** Governed release path: readiness run `35547295339` (all gates green including verify:ci 978/978 tests, bundle 574,865 B < 575,000 budget, D1 preview backup verified) → release run `35547386273` (D1 production migration applied, app + landing deployed via cutover scripts, smoke probes + alert delivery passed, attestation created and verified `ok: true` / `issues: []`, GitHub release `v0.7.1` published non-draft). Post-deploy verification: attestation SHA256 `3d3139…` matches between release artifact and local, public landing serves the v0.7.1 bundle (`index-BjFRmXZG.js`, 391,182 B), CF Access gate redirects to `red-queen-4dfa.cloudflareaccess.com` "Log in to selemene", probes `ok: true`. Supersedes v0.6.3 (tag retained, release run `34581173011`). Version bumped from 0.7.0 to 0.7.1 because the `v0.7.0` tag existed unreleased on the redesign merge commit `965e187`; five subsequent commits (Jev phases A-E, dead code cleanup, test fix, version bump) warranted a distinct version. Pre-deploy fix: `ui-home-contracts.test.mjs` still referenced deleted `PageTabs.tsx` — removed the read and its vocabulary assertion. `LOCATION_REMEDIATION_MANIFEST_JSON` Preview environment secret updated with new SHA (empty entries, no subjects pending remediation).

- 2026-09-19: **instrument-shell redesign (Phases 1-6) completed on `redesign/instrument-shell`.** Six phases shipped: (1) foundation tokens, radius/motion grammar, AppShell, ArrivalGate, error boundaries, NotFoundPage; (2) capsule masthead with scroll-shrink, CommandPalette lazy-chunked with cmdk, all-viewport identity; (3) URL-as-state for admin tabs, folio filters, node surfaces, view transitions; (4) FolioGallery tile grid, FolioFilters pills, CopyButton 4-state, ReadingActionBar; (5) ThresholdPage in-shell with progress rail, ContinuationDock replacing HomeJourneyRail, ConversationPage begin grid; (6) SettingsPage section IDs + nav pills, InstrumentDialog confirmations replacing window.confirm, compact session card. Brand locks held: void field, gold structure-only, parchment reading material, no light mode, tokens-only hex, graph-first IA, banned vocabulary resolved. Bundle: 574,793 bytes (207 under 575,000 budget). All 883 unit tests, 128 node contract tests, 7 source-shape contract scripts pass. Two open brand conflicts resolved: wordmark stays Cinzel/parchment as shipped (declared in tokens), single easing `cubic-bezier(0.19,1,0.22,1)` across all transitions. Ledger: `docs/ui/2026-09-19-instrument-shell-redesign-ledger.md`.
...
- 2026-09-21 | conjectured: the v0.7.0 tag on the redesign merge commit could be reused for the production release that includes five additional commits (Jev integration, cleanup, test fix).
  refuted by: the release workflow creates the tag via `gh release create`, which would collide with the existing tag at a different commit. Bumped to v0.7.1 instead.
  learned: unreleased tags from merge commits should be cleaned up immediately or the version bumped before release dispatch. The governed release path validates SHA→version→tag consistency end-to-end.
  criterion now: every production release has a distinct version; tags are created only by the release workflow; readiness + release are the two-step gate with same-SHA evidence binding.

- 2026-09-21 | conjectured: the `ui-home-contracts.test.mjs` would pass after dead code removal of `PageTabs.tsx`.
  refuted by: the contract test reads source files eagerly at import time — deleting `PageTabs.tsx` without updating the test caused `ENOENT` at module load, failing the entire `verify:ci` gate.
  learned: source-shape contract tests that `readFileSync` component files at import time must be updated atomically with component deletion. The test file is part of the component's deletion surface.
  criterion now: dead code removal PRs must grep `scripts/verify/*.test.mjs` for the deleted filename before committing.

- 2026-09-19 | conjectured: the inspo repo's design patterns (capsule masthead, command palette, URL-as-state, view transitions, tile gallery) could be adopted wholesale.
  refuted by: brand-lock audit — inspo assumes light/dark theming, gradient fills, toast notifications, and component libraries; Urania's void-only field, gold-as-structure constraint, and 575KB bundle budget required every pattern to be re-derived from tokens rather than ported. Capsule masthead became sticky (not fixed, per contract); palette lazy-chunked into its own chunk; gradients replaced with void + hairline; toasts replaced with inline 4-state feedback.
  learned: design borrowing that preserves brand constraints requires re-derivation, not adaptation. The patterns are structural (URL-as-state, lazy chunking, reduced-motion layers); the visual execution must come from the existing token system.
  criterion now: redesign ledger documents each adopted/rejected pattern with reason; baseline script confirms all vocabulary violations resolved; bundle budget holds at 574,793 of 575,000.
