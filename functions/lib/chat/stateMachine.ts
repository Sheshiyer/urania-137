/**
 * Functions-side port of the story state machine (Phase 1, W1-A).
 *
 * CANONICAL SOURCE: `src/lib/chat/stateMachine.ts`. This file is a line-for-
 * line port of that logic; the ONLY intentional differences are:
 *  - imports resolve against the local mirror `./types` (the Functions
 *    tsconfig cannot import from `src/`);
 *  - `BirthData` is mirrored locally below (`src/types/index.ts` has no
 *    counterpart in the chat mirror `functions/lib/chat/types.ts`);
 *  - `isStorySeed` is added (request-body guard for POST /api/chat/session —
 *    route-side concern, no SPA equivalent).
 *
 * Any behavioral change MUST be made in the canonical file first and ported
 * here; the parity suite (`functions/__tests__/chat.test.ts`) scripts the
 * same intakes as `src/lib/chat/stateMachine.test.ts`.
 *
 * Pure TypeScript — no React, no fetch, no DOM. The machine walks the chapter
 * router and each chapter's ask → validate → advance loop. One question at a
 * time; each valid user turn fills exactly one intake slot of
 * `ChatSessionState.intake` (a `Partial<AssetGenerateRequest>`).
 */
...
/** Mirror of `BirthData` in src/types/index.ts (deterministic-surface payload). */
...
/**
 * Functions-side addition (no SPA equivalent): request-body guard for
 * POST /api/chat/session. Structurally validates a client-supplied seed
 * before it is stored or used to derive capabilities.
 */
export function isStorySeed(x: unknown): x is StorySeed {
  ...
  switch (o.kind) {
    ...
    case 'workflow':
      return typeof o.workflowId === 'string' && o.workflowId.length > 0
    case 'engine':
      return typeof o.engineId === 'string' && o.engineId.length > 0
    case 'witness': {
      const min = o.minSubjects
      const max = o.maxSubjects
      return (
