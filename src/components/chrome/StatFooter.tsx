...
export function StatFooter({ stats }: { stats: Stat[] }) {
  return (
    <dl className="pointer-events-auto flex max-w-full items-stretch divide-x divide-gold/20 border-y border-gold/15 bg-void/30 backdrop-blur-[2px]">
      {stats.map((s) => (
        <div key={s.label} className="flex min-w-0 flex-col items-center px-3 py-2 sm:px-7 sm:py-2.5">
          <dt className="order-2 mt-1 truncate font-display text-[8px] uppercase tracking-[0.12em] text-silver/70 sm:text-[9px] sm:tracking-[0.26em]">{s.label}</dt>
