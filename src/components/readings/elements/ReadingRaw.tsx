...
export function ReadingRaw({ element }: { element: ReadingRawElement }) {
  ...
  return (
    <details className="console-card min-w-0 p-4" data-reading-element="raw">
      <summary id={id} className="min-h-11 cursor-pointer font-display text-[9px] uppercase tracking-[0.18em] text-gold marker:text-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold">
      ...
      <div
        className="mt-3 max-w-full overflow-auto"
        role="region"
        aria-label={`${element.title} privacy-filtered source`}
        tabIndex={0}
      >
        <pre className="max-h-80 min-w-max border border-gold/10 bg-void/80 p-3 font-mono text-[10px] leading-relaxed text-parchment/75">
