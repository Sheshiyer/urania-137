        ...
        {element.positions.map((position, index) => (
          <li key={position.id} className="border border-gold/15 bg-void/45 p-3">
            <div className="flex items-center gap-2">
              <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full border border-gold/35 font-mono text-[9px] tabular-nums text-gold">{index + 1}</span>
              <span className="font-display text-[9px] uppercase tracking-[0.16em] text-gold/65">{position.label}</span>
            </div>
            ...
            {position.detail && <p className="mt-2 text-pretty text-[10px] leading-relaxed text-silver/70">{position.detail}</p>}
          </li>
        ))}
      </ol>
    </ElementFrame>
