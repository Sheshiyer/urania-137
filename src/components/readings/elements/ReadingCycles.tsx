...
export function ReadingCycles({ element }: { element: ReadingCyclesElement }) {
  return (
    <ElementFrame element={element} encoding="Bar length uses the source scale; every value and phase is also written in text.">
      {element.targetLabel && <p className="mb-3 font-mono text-[9px] uppercase tracking-[0.14em] text-silver/60">Target · {element.targetLabel}</p>}
      <ul className="space-y-3">
        {element.cycles.map((cycle) => (
          <li key={cycle.id}>
            ...
            {cycle.detail && <p className="mt-1 text-[9px] text-silver/55">{cycle.detail}</p>}
          </li>
        ))}
      </ul>
    </ElementFrame>
