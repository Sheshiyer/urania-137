    ...
    >
      <div className="space-y-3" data-engine-artifact="media-artifact">
        {element.items.map((item) => {
          ...
          return (
            <article key={item.id} className="border border-gold/15 p-3" data-media-status={renderedStatus}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h4 className="text-sm text-reading-ink">{item.label}</h4>
                <span className="text-[9px] uppercase tracking-[0.12em] text-reading-muted">
              ...
              <p className="mt-3 text-xs text-reading-muted">{item.textEquivalent}</p>
