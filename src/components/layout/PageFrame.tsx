...
/** Art-deco circuit-board corner ornament with a rosette at the outer tip. */
function CornerOrnament({ className }: { className: string }) {
  return (
    <svg
      className={`pointer-events-none fixed z-10 h-40 w-40 text-gold/60 ${className}`}
      viewBox="0 0 140 140"
      fill="none"
      aria-hidden="true"
    >
      ...
      {/* corner rosette — the ringed sun at the frame's outer tip */}
      <circle cx="6" cy="6" r="4.5" stroke="currentColor" strokeWidth="0.8" strokeOpacity="0.9" />
...
function EdgeDiamond({ className }: { className: string }) {
  return <span className={`pointer-events-none fixed z-10 h-2 w-2 rotate-45 border border-gold/50 ${className}`} aria-hidden="true" />
}

export function PageFrame() {
  return (
    <>
      <div className="pointer-events-none fixed inset-3 rounded-sm border border-gold/20" aria-hidden="true" />
      <div className="pointer-events-none fixed inset-4 border border-gold/[0.07]" aria-hidden="true" />
      ...
      {/* Mid-edge diamonds on all four edges — the references close every side. */}
      <EdgeDiamond className="left-1/2 top-2 -translate-x-1/2" />
      <EdgeDiamond className="bottom-2 left-1/2 -translate-x-1/2" />
      <EdgeDiamond className="left-2 top-1/2 -translate-y-1/2" />
      <EdgeDiamond className="right-2 top-1/2 -translate-y-1/2" />
    </>
  )
}
