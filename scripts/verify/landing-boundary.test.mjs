...
test('landing build is a separate static artifact with one allowlisted exit', () => {
  ...
  assert.equal(existsSync(join(LANDING_OUTPUT, 'instrument', 'index.html')), true)
  assert.equal(existsSync(join(LANDING_OUTPUT, 'lenses', 'index.html')), true)
  assert.equal(existsSync(join(LANDING_OUTPUT, 'principles', 'index.html')), true)
  assert.equal(existsSync(join(LANDING_OUTPUT, 'enter', 'index.html')), true)
  ...
  assert.match(text, /urania\/hero-2k\.png/)
  assert.match(text, /\/instrument/)
  assert.match(text, /\/lenses/)
  assert.match(text, /\/principles/)
